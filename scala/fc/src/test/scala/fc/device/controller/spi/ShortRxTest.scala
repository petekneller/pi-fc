...
class ShortRxTest extends FlatSpec with Matchers with TypeCheckedTripleEquals with MockFactory {
  ...
  "ShortRx" should "fetch 2 8-bit registers and combine them into a 16-bit word" in {
    ...
    ShortRx(loByteRegister, hiByteRegister).read(device) should === (Right(0x1234.toShort))
  }
  ...
  it should "handle properly a negative signed value in the low byte" in {
    ...
    ShortRx(loByteRegister, hiByteRegister).read(device) should === (Right(0x1284.toShort))
  }
  ...
  it should "handle properly a negative signed value in the high byte" in {
    ...
    ShortRx(loByteRegister, hiByteRegister).read(device) should === (Right(0x8234.toShort))
  }
