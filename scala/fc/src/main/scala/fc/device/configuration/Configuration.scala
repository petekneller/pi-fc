...
case class MultiBitFlag[E <: FlagEnumeration](register: Register, hiBit: Int, numBits: Int, options: E) extends Configuration {
  ...
  def read(device: Address)(implicit controller: Controller { type Bus = device.Bus }): Either[DeviceException, E#T] = for {
  ...
  def write(device: Address, value: E#T)(implicit controller: Controller { type Bus = device.Bus }): Either[DeviceException, Unit] = for {
