...
class MacrosTest extends FlatSpec with Matchers with TypeCheckedTripleEquals {
  ...
  "the IOC macro" should "place the 'nr' identifier in the bottom 8 bits" in {
    val nr = rand.toByte
    "0x%x".format(IOC(IOC_NONE, rand.toByte, nr, classOf[Int]) & 0x000000FF) should === ("0x%x".format(nr.unsigned.toInt))
  }

  it should "place the 'type' identifier in the upper 8 of the bottom 16 bits" in {
    val `type` = rand.toByte
    "0x%x".format(IOC(IOC_NONE, `type`, rand.toByte, classOf[Int]) & 0x0000FF00) should === ("0x%x".format(`type`.unsigned << 8))
  }

  it should "place the 'size' in the lower 14 of the upper 16 bits" in {
    "0x%08x".format(IOC(IOC_NONE, rand.toByte, rand.toByte, classOf[Int]) & 0x3FFF0000) should === ("0x00040000")
    "0x%08x".format(IOC(IOC_NONE, rand.toByte, rand.toByte, classOf[Byte]) & 0x3FFF0000) should === ("0x00010000")
  }
